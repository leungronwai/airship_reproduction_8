...
def calculate_aero_forces_moments(
    ...
    args:
        ...
        delta_ELVL (float): left elevator deflection angle (rad)
        delta_ELVR (float): right elevator deflection angle (rad)
        use_casadi (bool): whether to use CasADi (instead of NumPy)

    return:
        tuple: (fa_BRF, ma_BRF) - return aerodynamic forces and moments vector
    """
...
def calculate_relative_velocity(v_airship_brf, V_wind_BRF):
    ...
    args:
        v_airship_brf: ground speed in body frame
        V_wind_BRF: wind speed in body frame

    return:
        tuple: (v_rel, u_rel, v_rel_body, w_rel) - relative wind speed vector and its components
    """
    ...
    if isinstance(v_rel, np.ndarray) and v_rel.ndim > 1:
        ...
        # handle ordinary array or CasADi vector
