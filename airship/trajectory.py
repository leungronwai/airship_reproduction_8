...
class Trajectory:
    ...
    def get_figure8_trajectory(self, t):
        ...
        yd_dot_plus = b * omega * (np.cos(omega * (t + dt_small)) * np.cos(omega * (t + dt_small)) - 
        ...
        yd_dot_minus = b * omega * (np.cos(omega * (t - dt_small)) * np.cos(omega * (t - dt_small)) - 
                                 np.sin(omega * (t - dt_small)) * np.sin(omega * (t - dt_small)))
    ...
    def get_lemniscate_trajectory(self, t):
        ...
        yd_dot_num_plus = a * (np.cos(theta_plus)**2 - np.sin(theta_plus)**2) * denom_plus - a * np.sin(theta_plus) * np.cos(theta_plus) * 2 * np.sin(theta_plus) * np.cos(theta_plus)
        ...
        yd_dot_num_minus = a * (np.cos(theta_minus)**2 - np.sin(theta_minus)**2) * denom_minus - a * np.sin(theta_minus) * np.cos(theta_minus) * 2 * np.sin(theta_minus) * np.cos(theta_minus)
