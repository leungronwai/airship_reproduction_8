...
"""
物理模型计算模块 - 包含质量、惯性和气动特性的计算 / Physical model calculation module - contains the calculation of mass, inertia, and aerodynamic characteristics
...
def calculate_added_mass_inertia(a1, a2, b, rho_air_):
    ...
    # 这里假设 a >= b，与图片中公式一致 / Assuming a >= b as consistent with the provided formulas.
    if b > a:
        print(
        ...
        # 对于扁椭球需要不同的公式或检查源文献 / Different formulas or source check needed for oblate case.
    ...
    if abs(a - b) < tolerance:
        ...
        if (1.0 - (b**2 / (a))) < 0:
            # 这理论上不应该在 a >= b 时发生，除非有数值误差 / This should not happen in a >= b unless there is a numerical error
            print(f"Warning: Eccentricity calculation issue (term = {term_inside_sqrt:.2e}). Setting e to 0.")
            _e = 0.0
            ...
            if abs(1.0 - _e) < tolerance:
                raise ValueError(" Eccentricity e approaches 1 (invalid geometry)") # 偏心率 e 接近 1 (b 接近 0)，几何形状无效。

            # 计算中间参数 f, g, alpha_prime, beta_prime / Calculate intermediate parameters f, g, alpha_prime, beta_prime
            ...
            if abs(e_cubed) < tolerance:
                ...
                raise ValueError(" Eccentricity e cubed is close to zero, cannot calculate g.") # 偏心率 e 的立方接近于零，无法计算 g。
            ...
            if abs(e_sq) < tolerance:
                raise ValueError(
                    " The square of eccentricity e is close to zero, beta_prime cannot be calculated." #偏心率 e 的平方接近于零，无法计算 beta_prime。
                )
            beta_ = (1.0 / e_sq) - (_g * f / 2.0)  # eq. 48

            # 计算惯性因子 k1, k2, k3 / Calculate inertia factors k1, k2, k3
            # k1 (Eq. 49)
            denominator_k1 = 2.0 - alpha_  # 分母 / denominator
            if abs(denominator_k1) < tolerance:
                raise ValueError(" Small denominator in k1 calculation") # 计算 k1 时分母接近零。/
            k1_ = - alpha_ / (2.0 - alpha_)  # eq. 49

            # k2 (Eq. 50)
            denominator_k2 = 2.0 - beta_
            if abs(denominator_k2) < tolerance:
                raise ValueError(" Small denominator in k2 calculation") #计算 k2 时分母接近零。/
            ...
            if abs(term2_den_k3) < tolerance:
                ...
                if abs(a - b) > tolerance:
                    raise ValueError(" Small denominator in k3 calculation.") # 计算 k3 时分母接近零 (非球体情况) /
