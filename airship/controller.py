...
class AnyController:
    def __init__(self):
        ...
        self.params = params  # 确保能访问params参数 / Ensure access to params
        self.rp_r = params.rp_r
        self.rp_l = params.rp_l
        
        # 添加PID控制器参数 / Add PID controller parameters
        ...
        # 姿态PID参数  / Attitude PID parameters
    ...
    def calculate_control(self, t, x, x_ref, x_ref_dot, u_thrust=None):
        """
        基于PID控制计算控制输入 / Calculate control input based on PID control
        ...
        参数:
            ...
            u_thrust: 初始推力参数 [T, μ, v], 如果为None则由控制器计算 / Initial thrust parameters [T, mu, v], if None, calculated by controller
            
        返回:
            tau: 6维控制向量 [Fx, Fy, Fz, Tx, Ty, Tz] / 6D control vector [Fx, Fy, Fz, Tx, Ty, Tz]
        """
        ...
        # 计算PID控制输出 (带前馈) / Calculate PID control output (with feedforward)
        ...
        # 将PID控制输出转换为推力参数和控制力矩 / Convert PID control output to thrust parameters and control torque
        if u_thrust is not None:
            # 使用提供的初始推力参数 / Use provided initial thrust parameters
            tau = self._thrust_params_to_tau(u_thrust)
            # 添加PID控制修正 / Add PID control correction
            tau[0:3] += F_pos
            tau[3:6] += T_att
        else:
...
class NMPCThrustController:
    ...
    def _build_nlp(self, X_sym, U_sym):
        ...
        for k in range(N):   # 构建预测时域内的优化问题 
            ...
            # 添加动力学约束： 使用离散时间动力学模型f_d，确保优化解满足系统动力学
