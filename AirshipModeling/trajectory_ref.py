...
import numpy as np
from AirshipModeling.trajectory_ref import R_zeta, R_y_inv
...
class Trajectory:
    ...
    def get_spiral_trajectory(self, t):
        ...
        # --- 内嵌辅助函数：计算指定时间的位置和姿态 ---
        def _calc_pos_att_at_time(time_val):
            """计算指定时间的位置和姿态，用于数值微分"""
            theta_val = omega * time_val
            # Position
            xd_val = r * np.cos(theta_val)
            yd_val = r * np.sin(theta_val)
            zd_val = h_max * (1 - np.exp(-theta_val / 10))
            zeta_d_val = np.array([xd_val, yd_val, zd_val])

            # Velocity (for attitude calculation)
            xd_dot_val = -r * omega * np.sin(theta_val)
            yd_dot_val = r * omega * np.cos(theta_val)
            zd_dot_val = h_max * (1 / 10) * np.exp(-theta_val / 10) * omega

            # Attitude
            phi_d_val = 0.0
            theta_d_val = np.arctan2(-zd_dot_val, np.sqrt(xd_dot_val**2 + yd_dot_val**2))
            psi_d_val = np.arctan2(yd_dot_val, xd_dot_val)
            gamma_d_val = np.array([phi_d_val, theta_d_val, psi_d_val])

            return zeta_d_val, gamma_d_val
        ...
        _, gamma_d_plus = _calc_pos_att_at_time(t + dt_small)
        _, gamma_d_minus = _calc_pos_att_at_time(t - dt_small)
