...
rho_air_at_altitude = rho_air # [kg/m^3] 空气密度 @ ~20km - Placeholder
...
try:
    ...
    k1_val, k2_val, _, _ = aero.calculate_added_mass_inertia_local() # 或者从其他来源获取

    # 计算气动系数
    AERO_COEFFS = aero.get_aero_coefficients(k1=k1_val, k2=k2_val)

except ValueError as e:
    print(f"初始化气动参数时出错: {e}")
    AERO_COEFFS = None # 或者设置默认值并停止

# 在 parameters.py 中，可以不再定义 Cx1 等变量，
# 或者将 AERO_COEFFS 字典保存在 params 模块中
...
def calculate_added_mass_inertia(a1, a2, b, rho_air):
    ...
    if abs(a - b) < tolerance:
        ...
        # k1_ = k2_ = k3_ = 0.5 (标准流体动力学结果)
        k1_ = 0.5
        k2_ = 0.5
        k3_ = 0.5
    else:
        # 计算偏心率 (Calculate eccentricity e - Eq. 44)
        # 确保 a^2 > 0 且 1 - (b^2 / a^2) >= 0
        a_sq = a**2
        term_inside_sqrt = 1.0 - (b**2 / a_sq)
        if (1.0 - (b**2 / (a**2))) < 0:
             # 这理论上不应该在 a >= b 时发生，除非有数值误差
             print(f"警告: 偏心率计算出现问题 (term = {term_inside_sqrt:.2e})。将 e 设为 0。")
             e = 0.0
             k1_ = k2_ = k3_ = 0.5 # 退化为球体情况
        else:
            e = np.sqrt(1.0 - (b**2 / (a**2)))

            # 避免 e 极其接近 1 (避免 f 中的除零)
            if abs(1.0 - e) < tolerance:
                 raise ValueError("偏心率 e 接近 1 (b 接近 0)，几何形状无效。")

            # 计算中间参数 f, g, alpha_prime, beta_prime
            ...
            if abs(term2_den_k3) < tolerance:
                ...
                if abs(a-b) > tolerance:
                     raise ValueError("计算 k3 时分母接近零 (非球体情况)。")
                else: # 如果是球体，分子也为零，极限应为 0.5
                     k3_ = 0.5
            else:
    ...
    _I0_prime = m_air * np.diag([0.0, k3_, k3_]) # 注意第一个元素是 0
...
M_inv = np.linalg.inv(M_cfg) # 预计算逆矩阵 (Pre-compute inverse)
...
# kb = [95e⁻⁰.⁰⁵ᵗ+5, 495e⁻⁰.⁰³ᵗ+5, 45e⁻⁰.⁰⁵ᵗ+5, 19e⁻⁰.⁰⁵ᵗ+0.1, 4.9e⁻⁰.⁰¹ᵗ+0.1, 4.9e⁻⁰.⁰¹ᵗ+0.1]^T # Typo in paper? 495, 45 seem large. Using adjusted interpretation
kb_params = {
...
# V_WIND_FUNC = get_wind_erf # 如果使用函数，则在这里指定